    ...
    function stopRec(useIt) {
          ...
          var fin = function () {
                  ...
                  fr.onloadend = function () {
                          ...
                          var b64 = c >= 0 ? d.slice(c + 1) : d;
                          fetch(CFG.BRAIN, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ via_voice: "1", audio_b64: b64, audio_mime: "audio/webm", session_id: S.id, submission_id: S.submission_id }) }).then(function (r) { return r.text(); }).then(function (tx) {
                                  typing(false);
                                  var st = parseState(tx);
                                  var tr = st.transcript || st.text || "";
                                  if (tr && typeof tr === "string" && tr.charAt(0) === "{") { try { var inner = JSON.parse(tr); tr = inner.text || inner.transcript || tr; } catch (e) {} }
                                  tr = (tr || "").trim();
                                  if (!tr) { addMsg("bot", T[S.lang].retry); return; }
                                  handleSend(tr, false);
                          }).catch(function () { typing(false); addMsg("bot", T[S.lang].retry); });
                  };
                  fr.onerror = function () { typing(false); addMsg("bot", T[S.lang].retry); };
